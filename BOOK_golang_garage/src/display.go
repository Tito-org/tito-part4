...
func displayHTMLPage(get *data, w http.ResponseWriter, r *http.Request) {
	...
	if checkEmpty == 0 {
		...
		refresh := "<html> <script> var timer = setTimeout(function() { window.location='https://shwrfr.com/tito/' }, 2000); </script> </html>"
		w.Write([]byte(fmt.Sprintf(imgBack)))
		w.Write([]byte(fmt.Sprintf(refresh)))
	} else {
		returnBut := "<html> <style>  .button-ret {border-radius: 8px; color: black; border: 2px solid black; background-color: rgba(191,191,191,0.5); padding: 15px 32px; cursor: pointer; transition-duration: 0.4s;}  .button-ret:hover { background-color: black; color: white; } </style> <body> <form action=\"https://shwrfr.com/tito/?\">  <input type=\"submit\" Value=\"Return\" class=\"button-ret\"> </form> </body> </html>"
